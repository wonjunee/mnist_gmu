...
def take_subimages3d(A, subimSize, stride, indices=False, random_sampling=0):
    """ 
    Take sub images from a 3D image
    ...
    for i in range(nsubs):
        for j in range(nsubs):
            [X,Y,D]=np.meshgrid(range(i,i+n), range(j,j+n), range(dataN))
            subinds_x.append(X.ravel())
            subinds_y.append(Y.ravel())
            subinds_d.append(D.ravel())
            
    subims = A[subinds_d, subinds_y, subinds_x]
    return subims
...
def take_subimages_new_stack(A, subimSize, stride, indices=False, random_sampling=0):
    """ 
    Take sub images from a 2D image
    and convert them to a 2D matrix
    """
...
def take_subimages_new_combined(A, subimSize, stride, indices=False, random_sampling=0):
    """ 
    Take sub images from a 2D image
    and convert them to a 2D matrix
    """
...
def take_subimages(image, subsize, stride, indices=False, random_sampling=0):
    """ 
    Take sub images from a 2D image
    and convert them to a 2D matrix
    """
    ...
    if len(image.shape) >= 3:
        col, row, depth = image.shape[:3]
    else:
        col, row = image.shape[:2]

    ### Calculate the number of subimages
    # if not taking random subimages
    if random_sampling == 0:
        num_sub = int(((col - subsize)/stride + 1) * ((row - subsize)/stride + 1))

        # create an empty array for the result
        result = np.zeros((subsize*subsize, num_sub), dtype=np.float64)

        ind = 0
        for offset_col in range(0, col, stride):
            end_col = offset_col + subsize
            if end_col > col:
                break
            else:
                for offset_row in range(0, row, stride):
                    end_row = offset_row + subsize
                    # if an end value exceeds the matrix
                    if end_row > row:
                        break
                    else:
                        # if indices is true then append indices at the end of rows
                        if indices:
                            subimage = image[offset_col:end_col, offset_row:end_row].ravel()
                            coord = np.array([offset_col, offset_row])
                            result[:, ind] = np.concatenate((subimage,coord))
                        else:
                            result[:, ind] = np.copy(image[offset_col:end_col, offset_row:end_row].ravel())
                        ind += 1

    # if taking random subimages
    else:
        ...
        random_cols = np.random.choice(col - subsize + 1, row_sub)
        random_rows = np.random.choice(row - subsize + 1, row_sub)

        ind = 0
        for offset_col in random_cols:
            end_col = offset_col + subsize
            for offset_row in random_rows:
                end_row = offset_row + subsize
                # if indices is true then append indices at the end of rows
                if indices:
                    subimage = image[offset_col:end_col, offset_row:end_row].ravel()
...
def take_subimages_3d(image, subsize, stride, indices=False):
    """ Take sub images from a 3D image
    and convert them to a 2D matrix"""

    if len(image.shape) != 3:
        print("------------------------------------------")
        print("           need to be 3d matrix           ")
        print("------------------------------------------")
        raise
    ...
    # create an empty array for the result
    # add two for coordinates if indices is true
    result = np.zeros((subsize*subsize*depth + add_row, num_sub), dtype=np.float64)
    for offset_col in range(0, col, stride):
        end_col = offset_col + subsize
        # if an end value exceeds the matrix, then break
        if end_col > col:
            break
        else:
            for offset_row in range(0, row, stride):
                ...
                if end_row > row:
                    ...
                    subimage = image[offset_col:end_col, offset_row:end_row, :].ravel()
                    if indices: # add indices at the end
                        coord = np.array([offset_row, offset_col])
                        subimage = np.concatenate((subimage,coord))
                    result[:, ind] = np.copy(subimage)
                    ind += 1
    # return subimages
    return result            
